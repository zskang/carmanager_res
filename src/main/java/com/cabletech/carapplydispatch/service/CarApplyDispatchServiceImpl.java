...
public class CarApplyDispatchServiceImpl extends BaseServiceImpl implements
	...
	private JmsSmParameter parameter = null;;
	private JmsSmParameter parameter2 = null;
	...
	public void sendAndUpdateNONECarState(CarApplyDispatchEntity entity,
		...
		parameter = JmsSmParameter.getInstanceForNeedResponse(
				mapp.get("code_user").toString(), mapp.get("WAITDISP")
						.toString());
	...
	public void sendAndUpdateWaitCheckState(CarApplyDispatchEntity entity,
			Map<String, Object> mapp) {
		parameter = JmsSmParameter.getInstanceForNeedResponse(
				mapp.get("code_user").toString(), mapp.get("WAITCHECK")
						.toString());
		parameter.setBusinessCode(mapp.get("buessinCode").toString());
		smSender.sendMessage(parameter);
		parameter2 = JmsSmParameter.getInstanceForNeedResponse(
				mapp.get("code_zz").toString(), mapp.get("WAITTOCHECK")
						.toString());
	...
	public void sendAndUpdateCANCELState(CarApplyDispatchEntity entity,
			Map<String, Object> mapp) {
		parameter = JmsSmParameter.getInstanceForNeedResponse(
				mapp.get("code_user").toString(), mapp.get("DISPBACK")
						.toString());
		parameter.setBusinessCode(mapp.get("buessinCode").toString());
		smSender.sendMessage(parameter);
		map.put("id", entity.getId());
	...
	public Map<String, Object> getStringMap(CarApplyDispatchEntity entity) {
		...
		flagString
				.append("  "
						+ baseInfoProvider.getPersonService()
								.getPersonInfo(entity.getApprover())
								.get("USERNAME").toString() + " 您好！")
				.append(entity.getUsername() + "于")
				.append(entity.getApply_date().substring(5,
						entity.getApply_date().length() - 5))
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.cabletech.carapplydispatch.service.CarApplyDispatchService#
	 * selectUserByPhone(java.lang.String)
	 */
	@Override
	public Map<String, Object> selectUserByPhone(String phone) {
		List<Map<String, Object>> list = carApplyDispatchMapper
				.selectUserByPhone2(phone);
